          ...
          <div>
           ...
           <?php
                wp_nav_menu([
                    ...
                    'container_class' => 'absolute py-5 lg:py-0 lg:px-4 xl:px-6 bg-white dark:bg-dark lg:dark:bg-transparent lg:bg-transparent shadow-lg rounded-lg max-w-[250px] w-full lg:max-w-full lg:w-full right-4 top-full hidden lg:block lg:static lg:shadow-none',
                    'menu_class'      => 'block lg:flex',
                    'fallback_cb'     => false,
                    'depth'           => 2, // чтобы поддержать вложенные подменю
                    'walker'          => new class extends Walker_Nav_Menu {
                        // Для добавления классов submenu-item и работы подменю как в статике
                    function start_lvl(&$output, $depth = 0, $args = null) {
                        $indent = str_repeat("\t", $depth);
                        $submenu_class = 'submenu hidden relative lg:absolute w-[250px] top-full lg:top-[110%] left-0 rounded-md lg:shadow-lg p-4 lg:block lg:opacity-0 lg:invisible group-hover:opacity-100 lg:group-hover:visible lg:group-hover:top-full bg-white dark:bg-dark transition-[top] duration-300';
                    ...
                    function start_el(&$output, $item, $depth=0, $args=null, $id=0) {
                        ...
                        // Добавляем класс submenu-item для пунктов с дочерними элементами
                            if (in_array('menu-item-has-children', $classes)) {
                                $classes[] = 'relative group submenu-item';
                                } else {
                        ...
                        $output .= $indent . '<li class="' . esc_attr($class_names) . '">';
                            ...
                            // Ссылка с классами
                        $link_classes = 'text-base text-dark dark:text-white group-hover:opacity-70 py-2 lg:py-6 lg:inline-flex lg:px-0 flex mx-8 lg:mr-0';
                            if ($depth > 0) {
                        $link_classes = 'block text-sm rounded py-[10px] px-4 text-dark dark:text-white hover:opacity-70';
                    }
                        ...
                        $output .= $title;
                            ...
                            // Добавляем стрелочку для меню с подменю (только первый уровень)
                        if (in_array('menu-item-has-children', $classes) && $depth === 0) {
                            $output .= '<span class="after:absolute after:w-2 after:h-2 after:border-b-2 after:border-r-2 after:border-current after:rotate-45 lg:after:right-0 after:right-1 after:top-1/2 after:translate-y-[-50%] after:mt-[-2px]"></span>';
                        }
