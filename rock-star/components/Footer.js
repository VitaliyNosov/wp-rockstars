...
const GET_FOOTER_MENUS = gql`
  query GetFooterMenus {
    menu1: menus(where: {slug: "footer-block-one-menu"}) {
      nodes {
        menuItems {
          nodes {
            id
    ...
    menu2: menus(where: {slug: "footer-block-two-menu"}) {
      nodes {
        menuItems {
          nodes {
            id
    ...
    menu3: menus(where: {slug: "footer-block-three-menu"}) {
      nodes {
        menuItems {
          nodes {
            id
...
const Footer = () => {
    const { data, loading, error } = useQuery(GET_FOOTER_MENUS, { client });

    const menu1Items = data?.menu1?.nodes[0]?.menuItems?.nodes || [];
    const menu2Items = data?.menu2?.nodes[0]?.menuItems?.nodes || [];
    const menu3Items = data?.menu3?.nodes[0]?.menuItems?.nodes || [];
                ...
                <div className="container mx-auto">
                    <div className="flex flex-wrap mx-[-16px]">
                        <div className="w-full md:w-1/2 lg:w-4/12 xl:w-5/12 px-4">
                            <div className="mb-16 max-w-[360px]">
                                <a href="/" className="inline-block mb-8">
                                    <img src="http://localhost:8081/wp-content/uploads/2025/06/logo-2.svg" alt="logo" className="w-full dark:hidden" />
                                    <img src="http://localhost:8081/wp-content/uploads/2025/06/logo.svg" alt="logo" className="w-full hidden dark:block" />
                                </a>
                                ...
                                <div className="flex items-center">
                                    <a href="#" aria-label="social-link" className="text-[#CED3F6] hover:text-primary mr-6" onClick={(e) => e.preventDefault()}>
                        ...
                        <div className="w-full sm:w-1/2 md:w-1/2 lg:w-2/12 xl:w-2/12 px-4">
                            <div className="mb-16">
                                <h2 className="font-bold text-black dark:text-white text-xl mb-10">Useful Links</h2>
                                <ul>
                                    {menu1Items.map((item) => (
                                        <li key={item.id}>
                                            <Link href={item.uri} className="text-base font-medium inline-block text-body-color mb-4 hover:text-primary">
                                                {item.label}
                                            </Link>
                                        </li>
                                    ))}
                        ...
                        <div className="w-full sm:w-1/2 md:w-1/2 lg:w-2/12 xl:w-2/12 px-4">
                            <div className="mb-16">
                                <h2 className="font-bold text-black dark:text-white text-xl mb-10">Terms</h2>
                                <ul>
                                    {menu2Items.map((item) => (
                                        <li key={item.id}>
                                            <Link href={item.uri} className="text-base font-medium inline-block text-body-color mb-4 hover:text-primary">
                        ...
                        <div className="w-full md:w-1/2 lg:w-4/12 xl:w-3/12 px-4">
                            <div className="mb-16">
                                <h2 className="font-bold text-black dark:text-white text-xl mb-10">Support & Help</h2>
                                <ul>
                                    {menu3Items.map((item) => (
                                        <li key={item.id}>
                                            <Link href={item.uri} className="text-base font-medium inline-block text-body-color mb-4 hover:text-primary">
