...
const HeroSection = ({ data }) => {
    ...
    // Title Logic: Split words, color last word blue
    ...
    const heroImage = data.heroImage || 'https://images.unsplash.com/photo-1665686377065-08ba896d16fd?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=700&h=800&q=80';

    return (
        <div className="landing-header-landing-section max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8">
            {/* Grid */}
            <div className="grid md:grid-cols-2 gap-4 md:gap-8 xl:gap-20 md:items-center">
                <div>
                    <h1 className="block text-3xl font-bold text-gray-800 sm:text-4xl lg:text-6xl lg:leading-tight dark:text-white wow fadeInUp" data-wow-delay=".2s">
                        {renderTitle(data.heroTitle)}
                    </h1>
                    <p className="mt-3 text-lg dark:text-white dark:text-neutral-400 wow fadeInUp" data-wow-delay=".4s">
                        {data.heroDescription}
                    </p>

                    {/* Buttons */}
                    <div className="mt-7 grid gap-3 w-full sm:inline-flex wow fadeInUp" data-wow-delay=".6s">
                        {data.heroBtn1Text && (
                            <a className="py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-hidden focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" href={data.heroBtn1Url || '#'}>
                                {data.heroBtn1Text}
                        ...
                        {data.heroBtn2Text && (
                            <a className={`${styles.customHoverButtonLanding} py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-2xs hover:bg-gray-50 focus:outline-hidden focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-transparent dark:border-gray-700 dark:text-white dark:hover:bg-neutral-800 dark:focus:bg-neutral-800`} href={data.heroBtn2Url || '#'}>
                                {data.heroBtn2Text}
                            </a>
                        )}
                    </div>
                    ...
                    <div className="mt-6 lg:mt-10 grid grid-cols-2 gap-x-5">
                        ...
                        <div className="py-5">
                            <div className="flex gap-x-1">
                                {[...Array(5)].map((_, i) => (
                                    <svg key={i} className="size-4 dark:text-white dark:text-neutral-200" width="51" height="51" viewBox="0 0 51 51" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <p className="mt-3 text-sm dark:text-white dark:text-neutral-200">
                                <span className="font-bold">{data.heroReview1Rating || '4.6'}</span> /5 - from {data.heroReview1Count || '12k reviews'}
                            </p>

                            <div className="mt-5">
                                {/* Star Logo 1 */}
                                <svg className="h-auto w-16 dark:text-white dark:text-white" width="80" height="27" viewBox="0 0 80 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="relative ms-4 wow fadeInUp" data-wow-delay=".8s">
