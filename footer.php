    ...
    <div class="container">
      <div class="flex flex-wrap mx-[-16px]">
        <div class="w-full md:w-1/2 lg:w-4/12 xl:w-5/12 px-4">
          <div class="mb-16 max-w-[360px]">
            <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="inline-block mb-8">
              <?php
              $rock_stars_custom_logo_id = get_theme_mod( 'custom_logo' );
              $rock_stars_logo = wp_get_attachment_image_src( $rock_stars_custom_logo_id, 'full' );
              if ( has_custom_logo() ) {
                  echo '<img src="' . esc_url( $rock_stars_logo[0] ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" class="w-full">';
              } else {
                  ?>
            ...
            <p class="text-body-color text-base font-medium leading-relaxed mb-9">
              <?php esc_html_e( 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer lobortis.', 'rock-stars' ); ?>
            </p>
            <div class="flex items-center">
              <a href="javascript:void(0)" aria-label="<?php esc_attr_e( 'social-link', 'rock-stars' ); ?>" class="text-[#CED3F6] hover:text-primary mr-6">
        ...
        <div class="w-full sm:w-1/2 md:w-1/2 lg:w-2/12 xl:w-2/12 px-4">
          <div class="mb-16">
            <h2 class="font-bold text-black dark:text-white text-xl mb-10">
                <?php esc_html_e( 'Useful Links', 'rock-stars' ); ?>
            </h2>
            <?php
                wp_nav_menu([
                  'theme_location' => 'footer-one',
                  'container' => false,
                  'menu_class' => '', // можно задать класс ul, если нужно
                  'items_wrap' => '<ul>%3$s</ul>',
                  'walker' => new class extends Walker_Nav_Menu {
                  function start_el(&$output, $item, $depth = 0, $args = null, $id = 0) {
                    $classes = 'text-base font-medium inline-block text-body-color mb-4 hover:text-primary';
        ...
        <div class="w-full sm:w-1/2 md:w-1/2 lg:w-2/12 xl:w-2/12 px-4">
          <div class="mb-16">
            <h2 class="font-bold text-black dark:text-white text-xl mb-10">
                <?php esc_html_e( 'Terms', 'rock-stars' ); ?>
            </h2>
            <?php
                wp_nav_menu([
        ...
        <div class="w-full md:w-1/2 lg:w-4/12 xl:w-3/12 px-4">
          <div class="mb-16">
            <h2 class="font-bold text-black dark:text-white text-xl mb-10">
                <?php esc_html_e( 'Support & Help', 'rock-stars' ); ?>
            </h2>
            <?php
                wp_nav_menu([
    ...
    <div class="py-8 bg-primary bg-opacity-10">
      <div class="container">
        <p class="text-body-color dark:text-white text-base text-center">
            ...
            printf( esc_html__( '&copy; %s Rock Stars. All rights reserved.', 'rock-stars' ), esc_html( date( 'Y' ) ) );
            ?>
        </p>
      </div>
    </div>
